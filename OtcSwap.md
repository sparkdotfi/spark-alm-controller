...
-   All assets are tracking the same underlying. In other words, there will only be USD stablecoins. We treat the value of these assets the same (i.e. 1 USDT = 1 USDC). No yield-bearing versions, just 1:1.
...
-   The maximum loss by the protocol is limited to `amountIn * maxSlippage + rechargeRate * days`, per whitelisted exchange address.
...
-   **OTC-Specific Slippage**: For OTC swaps, the system uses the same slippage mechanism but applies it to the recharge calculation upon claiming funds, to determine when (and the quantity of) additional funds can be sent for a new OTC trade with that exchange

### Recharge Rate Assumptions

The recharge rate mechanism ensures that funds gradually become available for new OTC swaps over time, providing a safety mechanism against potential losses:

-   **Linear Recharge**: The recharge rate operates linearly over time using the formula: `(block.timestamp - sentTimestamp) * rechargeRate`
...
-   **Slippage Integration**: The recharge mechanism works in conjunction with slippage protection - additional funds can only be sent when `totalClaimedFromLastSend + ((block.timestamp - lastSentTimestamp) * rechargeRate) >= lastSent * maxSlippage`. Notice that if `totalClaimedFromLastSend` is already within the slippage bounds given `lastSent` then a new OTC swap can immediately be performed. If not, sufficient time must elapse for the recharge term to "heal" the previous greater than expected slippage loss, to allow for a new OTC swap.888
